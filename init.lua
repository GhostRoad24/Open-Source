...
function msgplayer(ply,msg)
    ...
    net.Start("dstaffmessage")
    net.WriteString(msg)
    net.Send(ply)
    return
end
hook.Add("PhysgunPickup", "DeltasCarPhysgun", function(ply,ent)
    if not ent:IsVehicle() or ent:GetClass():lower() ~= "prop_vehicle_jeep" then return end
    if (!ply:IsStaff()) then return false end
    return true
end)

hook.Add("OnPhysgunFreeze", "DeltasCarKick", function(weapon,physobj,ent,ply)
    if (!ply:IsStaff()) then return end
    if (!ent:IsVehicle()) then return end
    for i,v in pairs(ent:VC_getPlayers()) do
        msgplayer(v, "You're being kicked out of the vehicle by a staff member.")
    end
    ent:VC_clearSeats()
    msgplayer(ply, "Kicked all players out of vehicle")
end)
